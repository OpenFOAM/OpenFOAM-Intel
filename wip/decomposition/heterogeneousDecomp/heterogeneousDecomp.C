...
namespace Foam
{
    defineTypeNameAndDebug(heterogeneousDecomp, 0);
...
void Foam::heterogeneousDecomp::subsetGlobalCellCells
(
...
void Foam::heterogeneousDecomp::decompose
(
    const labelListList& pointPoints,
    const pointField& points,
    const scalarField& pointWeights,
    labelList& pointBounds,
...
)
{
    ...
    // update bounding cells
    label count = 0;
    forAll(dist, i)
    {
        label domain = -1;
        forAll(pointPoints[i], pI)
        {
            label d = pointPoints[i][pI];
            if(domain == -1)
            {
                domain = dist[d];
            }
            else if(dist[d] != domain)
            {
                label orig = pointMap[i];
                boundingCells[orig] = 1;
                ++count;
                break;
            }
        }
    }
    Info << "Number of bounding cells in partition is " << count << "(out of " << dist.size() << ")" << endl;
    ...
    if (levelI == 0)// != methods_.size()-1)
    {
        ...
        // Set finalDecomp for new levels of decomposition
        labelList myDecomp(n, 0);
		for (domainI = 1; domainI < n; domainI++)
		{
			myDecomp[domainI] = myDecomp[domainI-1] + methods_[domainI].nDomains();
		}
		forAll(finalDecomp, i)
		{
			finalDecomp[i] = myDecomp[finalDecomp[i]];
		}
        ...
        for (domainI = 0; domainI < n; domainI++)
        {
            ...
            labelList subBoundingPoints(subPoints.size(), 0);
            forAll(subPointMap, i)
            {
                label orig = subPointMap[i];
                subBoundingPoints[i] = boundingCells[orig];
            }
...
Foam::heterogeneousDecomp::heterogeneousDecomp(const dictionary& decompositionDict)
:
    ...
    methodsDict_(decompositionDict_.subDict(typeName + "Coeffs"))
{
    ...
    Info<< "    decomposing between types using " << methods_[0].type()
            << " for " << methods_[0].nDomains() << " types." << endl;
	for(i = 1; i < methods_.size(); ++i)
    {
        Info<< "    type " << i << " decomposing with " << methods_[i].type()
    ...
    if (n != nDomains())
    {
        FatalErrorIn("heterogeneousDecomp::heterogeneousDecomp(const dictionary&)")
...
bool Foam::heterogeneousDecomp::parallelAware() const
{
    forAll(methods_, i)
    {
...
Foam::labelList Foam::heterogeneousDecomp::decompose
(
    const polyMesh& mesh,
    const pointField& cc,
...
)
{
    ...
    labelList boundingCells(cc.size(), 0);

    const labelList& faceNeighbour = mesh.faceNeighbour();
    const labelList& faceOwner = mesh.faceOwner();
    for(label n = faceNeighbour.size(); n < faceOwner.size(); ++n)
    {
        boundingCells[faceOwner[n]] = 1;
    }
